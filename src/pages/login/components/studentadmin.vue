...
export default {
  ...
  methods:{
    ...
    getAll(){
      ...
      this.$http.post(this.$store.state.url+'pc/login/doregister', {orgid:this.schoolvalue,username:this.isname,collegename:this.branchvalue,context:'',accountname:this.isphone,smscode:this.iscode,xytoken:admintokeed,password:this.issurepassword,roleid:1},{emulateJSON: true,headers:{'xytoken':admintokeed}}).then((response) => {
        this.info = JSON.stringify(response.data, null, 4);
        if(response.data.result == "success"){
          this.$notify({
            title: '成功',
